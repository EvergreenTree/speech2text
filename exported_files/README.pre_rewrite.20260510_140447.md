# Speech2Text under compute constraints — zh-CN ↔ fr-FR
...
4. **Qwen3-ASR was added as a counterpoint, not just an extra row.** It shows
   how much the conclusion depends on backbone quality, pre-training mix, and
   the evaluation slice.

This repo bundles three tracks under one roof: the original Whisper
fine-tuning work, the `asr_bench` baseline benchmark, and the Qwen3-ASR pilot.
The earlier zh-CN Whisper run lives intact under `archive_zh/`; the present
fr-FR Whisper run is in `outputs/`. Tiny was added later to control for the
*gap-to-ceiling* effect discussed in §3.3.

The benchmark plots now include Qwen3-ASR `0.6B` and `1.7B` baseline and
fine-tune points on fixed `dev100` slices. The main `asr_bench` curves remain
the original benchmark runs; the overlays are for directional comparison
rather than strict leaderboard ranking.
...
I added a parallel **Qwen3-ASR** path under [`/data/speech2text/Qwen3-ASR/finetuning`](/data/speech2text/Qwen3-ASR/finetuning):

* [`prepare_qwen3_asr_data.py`](/data/speech2text/Qwen3-ASR/finetuning/prepare_qwen3_asr_data.py) exports the **same two datasets and split sizes** used in this README into JSONL + 16 kHz WAV:
  `fleurs-fr` = 3 193 / 289 / 500, `cv21-zh` = 4 000 / 300 / 500.
* [`qwen3_asr_sft.py`](/data/speech2text/Qwen3-ASR/finetuning/qwen3_asr_sft.py) now supports `--mode lora|full` on top of the upstream Qwen recipe, with PEFT LoRA, checkpoint copying for inferable saves, and gradient-checkpointing fixes for Qwen's outer wrapper.
* [`eval_qwen3_asr.py`](/data/speech2text/Qwen3-ASR/finetuning/eval_qwen3_asr.py) scores Qwen checkpoints with the same French / Chinese normalization logic used elsewhere here.
* [`run_qwen3_matrix.sh`](/data/speech2text/Qwen3-ASR/finetuning/run_qwen3_matrix.sh) wires the full matrix together.

Environment note: the Qwen code path had to be run from `/data/venv` with
...
```bash
# Install (reuse /data/venv on the test machine)
